...
class EvaluationsDistributionTest extends TestCase
{
    ...
    function testGlobalCommitteeFiltersWithSelectionFieldsForSecondEvaluationPhase()
    {
        $admin = $this->userDirector->createUser('admin');
        $this->login($admin);
        ...
        $opportunity = $this->opportunityBuilder
            ...
            ->firstPhase()
                ...
                ->done()
            ->save()
            // Primeira fase de avaliação (simples, sem filtros)
            ->addEvaluationPhase(EvaluationMethods::simple)
                ->setEvaluationPeriod(new ConcurrentEndingAfter)
                ->save()
                ->done()
            // Segunda fase de avaliação (com filtros globais de seleção)
            ->addEvaluationPhase(EvaluationMethods::simple)
                ->setEvaluationPeriod(new ConcurrentEndingAfter)
                ->setCommitteeValuersPerRegistration('committee-select', 1)
                ->setCommitteeFilterField('committee-select', 'campo_select', ['A'])
        ...
        // Aprova todas as inscrições para que possam ser propagadas para a próxima fase
        foreach ($registrations as $registration) {
            $registration->setStatusToApproved(true);
        }

        // Propaga as inscrições para a segunda fase de avaliação
        $opportunity->nextPhase->syncRegistrations($registrations);
        $this->processJobs();

        // Redistribui as avaliações na segunda fase
        $opportunity->nextPhase->evaluationMethodConfiguration->redistributeCommitteeRegistrations();
        ...
        // committee-select deve avaliar apenas inscrições com campo_select = 'A' (2 inscrições)
        $select_evaluations = $conn->fetchScalar(
            "SELECT COUNT(*) FROM evaluations WHERE valuer_committee = :committee AND opportunity_id = :opportunity_id",
            ['committee' => 'committee-select', 'opportunity_id' => $opportunity->nextPhase->id]
        );
        $this->assertEquals(
            2,
            $select_evaluations,
            'Garantindo que o comitê filtrado por campo select recebeu apenas as inscrições com valor A na segunda fase'
        );

        // committee-mult deve avaliar apenas inscrições onde campo_mult contém 'X' (2 inscrições: 0 e 2)
        $mult_evaluations = $conn->fetchScalar(
            "SELECT COUNT(*) FROM evaluations WHERE valuer_committee = :committee AND opportunity_id = :opportunity_id",
            ['committee' => 'committee-mult', 'opportunity_id' => $opportunity->nextPhase->id]
        );
        $this->assertEquals(
            2,
            $mult_evaluations,
            'Garantindo que o comitê filtrado por campo checkboxes recebeu apenas as inscrições contendo X na segunda fase'
        );

        // committee-bool deve avaliar apenas inscrições com campo_bool true (3 inscrições: 0, 2 e 3)
        $bool_evaluations = $conn->fetchScalar(
            "SELECT COUNT(*) FROM evaluations WHERE valuer_committee = :committee AND opportunity_id = :opportunity_id",
            ['committee' => 'committee-bool', 'opportunity_id' => $opportunity->nextPhase->id]
        );
        $this->assertEquals(
            3,
            $bool_evaluations,
