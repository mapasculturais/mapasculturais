...
<template v-if="results" v-for="section in results" class="faq-accordion">
    <div v-if="index" class="faq-accordion__contexts">
        <h2 v-html="section.title" class="bold has-color is-primary">></h2>
    ...
    <template v-if="section.contexts.length > 0" v-for="context in section.contexts">
        <div class="faq-accordion__contexts">
            <mc-title tag="h2" v-html="context.title" class="bold has-color is-primary"></mc-title>
...
<div v-if="results==0" class="faq-accordion__noresults">
    ...
    <a class="nounderline" href="<?= $this->controller->createUrl('index'); ?>"><label class="bold has-color is-primary faq-accordion__backhelp"><?= i::__('VOLTAR À AJUDA') ?></label></a>
</div>