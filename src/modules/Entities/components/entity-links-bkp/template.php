...
<div v-if="entity.metalists.links || editable" :class="['entity-links', classes]">
    ...
    <ul v-if="entity.metalists.links" class="entity-links__links">
        <li class="entity-links__links--item" v-for="metalist in entity.metalists.links">
            ...
            <div v-if="editable" class="entity-links__actions">
                <mc-confirm-button @confirm="metalist.delete()">
                    <template #button="{open}">
                        <button @click="open()" class="button button--outline entity-links__action-button danger__color danger__border">
    ...
    <div class="entity-links__list" v-if="editable">
        ...
        <div v-for="(link, index) in newLinks" class="entity-links__item">
            ...
            <div class="entity-links__buttons">
                <mc-confirm-button @confirm="remove(index)">
                    <template #button="{open}">
        ...
        <div class="entity-links__add-button">
            <mc-popover v-if="editable" title="<?php i::_e('Adicionar Link') ?>" openside="down-right" @open="metalist = {title: '', value: ''}">
                ...
                <template #default="{popover, close}">
                    <form @submit="create(popover, close); $event.preventDefault();" class="entity-links__newLink">
                        <div class="grid-12">
                            <div class="col-12">
                                <h1> <?php i::_e('Adicionar link') ?></h1>
                                <div class="field">
                                    <label><?php i::_e('URL do link') ?></label>
                                    <input v-model="metalist.value" class="input" type="url" name="newLink" />
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="field">
                                    <label><?php i::_e('Descrição do link') ?></label>
                                    <input v-model="metalist.title" class="input" type="text" name="newLinkTitle" />
                                </div>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <button class="col-6 button button--primary" type="submit"> <?php i::_e("adicionar link") ?> </button>
                            </div>
                        </div>
                    </form>
                </template>
