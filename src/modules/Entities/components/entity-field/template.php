...
<div v-if="propExists()" class="field" :class="[{error: hasErrors}, {disabled: readonly || disabled}, classes]" :data-field="prop">
    ...
    <div class="field__input">
        <slot name="input">
            ...
            <div v-else-if="is('socialMedia')" class="entity-social-media__input-wrapper">
                <mc-icon v-if="icon" :name="icon"></mc-icon>
                <input  :value="value" :id="propId" :name="prop" :type="fieldType" @input="change($event)" @blur="change($event,true)" autocomplete="off" :placeholder="placeholder || description?.placeholder" :disabled="readonly || disabled" :readonly="readonly">
            </div>
            ...
            <div class="field__group">
                    <template v-if="isMultiSelect()">
                        <mc-multiselect :placeholder="placeholder || description?.placeholder" @selected="change($event)" :model="selectedOptions[prop]" :items="description.options" #default="{popover,setFilter}" :max-options="maxOptions" :preserve-order="preserveOrder" :disabled="readonly || disabled" hide-filter hide-button></mc-multiselect>

                        <mc-tag-list :tags="selectedOptions[prop]" :labels="description?.options" classes="opportunity__background" @remove="change($event)" :editable="!(readonly || disabled)"></mc-tag-list>
                    </template>
