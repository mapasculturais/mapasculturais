...
');
?>
<div :class="classes" v-if="file || editable" class="files-list">
    <label class="files-list__title"> {{title}} </label>

    <ul v-if="file" class="files-list__list">
        <li class="files-list__list--item">
            <a class="files-list__list--item-link" :download="file.name" :href="file.url">
                <mc-icon name="download" :class="'is-' + entity.__objectType"></mc-icon>
