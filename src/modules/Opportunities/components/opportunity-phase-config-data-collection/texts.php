<?php
use MapasCulturais\i;

return [
    'confirma exclusao de fase' => i::esc_attr__('Confirma a execução da ação?'),
    'confirma exclusao de fase de prestacao' => i::esc_attr__('A fase de avaliação também será excluída. Confirma a execução da ação?'),
    'nao foi possivel remover fase' => i::__('Não foi possível remover a fase'),
    'alerta fluxo continuo sem data final' => i::__('Este edital está em fluxo contínuo sem data final. As inscrições permanecem abertas até que uma data de término seja definida.'),
    'habilitar data final das inscricoes' => i::__('Definir data final das inscrições'),
    'confirmar habilitar data final' => i::esc_attr__('Ao confirmar, o edital passará a ter data final de inscrições em todas as fases. Você poderá editar as datas de término de cada fase. Deseja continuar?'),
    'data final habilitada com sucesso' => i::__('Data final das inscrições habilitada. Defina as datas de término das fases.'),
    'nao foi possivel habilitar data final' => i::__('Não foi possível habilitar a data final das inscrições'),
];