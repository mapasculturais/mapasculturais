...
<div class="tecnical-evaluation-form">
        ...
        <div class="bold tecnical-evaluation-form__criterion" v-for="criterion in section.criteria" :key="criterion.id">
            <div class="field tecnical-evaluation-form__maxScore grid-12">
                <label> 
                    ...
                    <input class="maxScore-input" v-if="permetions && isEditable" v-model="formData.data[criterion.id]" min="0" step="0.1" type="number" @input="handleInput(sectionIndex, criterion.id)">
                    <input class="maxScore-input" v-if="!isEditable || !permetions" disabled min="0" step="0.1" type="number" :value="formData.data[criterion.id]" @input="handleInput(sectionIndex, criterion.id)">
                </label>
            </div>
            <div class="grid-12">
    ...
    <div class="tecnical-evaluation-form__textarea">
        ...
        <textarea v-if="permetions  && isEditable" v-model="formData.data.obs"></textarea>
        <textarea v-if="!permetions || !isEditable" disabled>{{formData.data.obs}}</textarea>
    </div>
    ...
    <div class="tecnical-evaluation-form__viability-radio-group" v-if="enableViability">
        ...
        <label>
            <span v-if="permetions  && isEditable"> <input v-model="formData.data.viability" type="radio" name="confirmation" value='valid' /> <?php i::_e('Sim') ?> </span>
            <span v-if="!permetions || !isEditable"> <input type="radio" name="confirmation" value="valid" disabled :checked="formData.data.viability == 'valid'"/> <?php i::_e('Sim') ?> </span>
        </label>
        <label>
            <span v-if="permetions  && isEditable"> <input v-model="formData.data.viability" type="radio" name="confirmation" value='invalid' /> <?php i::_e('Não') ?> </span>
            <span v-if="!permetions || !isEditable"> <input type="radio" name="confirmation" value="invalid" disabled :checked="formData.data.viability == 'invalid'"/> <?php i::_e('Não') ?> </span>
        </label>
    </div>
