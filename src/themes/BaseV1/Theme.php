                ...
                if($next_id = $registration->nextPhaseRegistrationId){
                    ...
                    if ($next_phase_registration) {
                        if($next_phase_registration->canUser('view')){
                            $this->part('opportunity-phases/next-phase-registration-link', ['next_phase_registration' => $next_phase_registration, 'registration' => $registration]);
                        }
                    }
                }
            }
    ...
    //
            ...
            foreach($registrations as $i => $reg){
                ...
                if (count($opportunity->registrationFieldConfigurations) || count($opportunity->registrationFileConfigurations)) {
                    ...
                    if (count($opportunity->registrationFieldConfigurations) > 0) {
                        $section_divisor->step = $opportunity->registrationFieldConfigurations[0]->step;
                    } else {
                        $section_divisor->step = count($opportunity->registrationFileConfigurations) > 0 ? 
                            $opportunity->registrationFileConfigurations[0]->step : 1;
                    }
