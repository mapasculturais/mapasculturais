		...
		<div class="controles">
			<?php if($this->controller->id == 'agent' && $entity->user->id != $app->user->id && $entity->id == $entity->user->profile->id && $entity->user->canUser('addRole')): ?>
                <div id="funcao-do-agente" class="dropdown">
                    ...
                    if($entity->user->is('superAdmin')) echo '<span data-role="superAdmin">' . $app->getRoleName('superAdmin') . '</span>';
                    elseif($entity->user->is('admin'))  echo '<span data-role="admin">' . $app->getRoleName('admin') . '</span>';
                    elseif($entity->user->is('staff'))  echo '<span data-role="staff">' . $app->getRoleName('staff') . '</span>';
                    else echo "<span>Normal</span>";
                    ?> </div>
