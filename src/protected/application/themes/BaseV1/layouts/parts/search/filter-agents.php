<div id="filter-agents" class="entity-filter clearfix" ng-show="data.global.filterEntity === 'agent'">
    ...
    <div ng-show="showSearch()">
        ...
        <div class="filter verified-filter search-filter--acessibilidade">
            <a class="hltip btn btn-verified" ng-class="{'selected':data.agent.isVerified}" title="Exibir somente <?php $this->dict('search: verified results') ?>" ng-click="toggleVerified('agent')"><?php $this->dict('search: verified results') ?></a>
        </div>
        <!-- div.verified-filter -->
