...
<div ng-controller="ChatController" ng-if="<?= $thread_id ?>" ng-hide="data.messages.length == 0 && isChatClosed()" ng-init="init(<?= $thread_id ?>)" class="chat">
    ...
    <footer ng-if="!isChatClosed()" ng-style="{'border-top-width': (data.messages.length == 0) ? '0px' : '1px'}">
        <textarea class="new-message" ng-keypress="handleCtrlEnterAction($event)" ng-focus="chatIsFocused()" ng-blur="data.chatFocusTime = 60000" ng-model="data.newMessage" ng-style="{'height': (!data.newMessage) ? 'auto' : ''}" placeholder="<?php i::_e("Escreva uma mensagem"); ?>"></textarea>
