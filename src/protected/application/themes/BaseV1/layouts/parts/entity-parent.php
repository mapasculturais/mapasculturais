<?php if(!isset($disable_editable) && $this->isEditable() && $entity->canUser('modifyParent')): ?> 
    ...
    <?php else:?>
        <?php if($entity instanceof MapasCulturais\Entities\Project): ?>
            <span  class="js-search js-include-editable"
                   ...
                   data-emptytext="<?php \MapasCulturais\i::_e("Selecionar projeto pai");?>"
                   data-search-box-width="400px"
                   data-search-box-placeholder="<?php \MapasCulturais\i::_e("Selecionar projeto pai");?>"
                   ...
                   data-allow-clear="1"
                   title="<?php \MapasCulturais\i::_e("Selecionar projeto pai");?>"
        ...
        <?php elseif($entity instanceof MapasCulturais\Entities\Space): ?>
            <span  class="js-search js-include-editable"
                    data-field-name='parentId'
                    data-emptytext="<?php \MapasCulturais\i::_e("Selecionar");?> <?php $this->dict('entities: parent space') ?>"
                    data-search-box-width="400px"
                    data-search-box-placeholder="<?php \MapasCulturais\i::_e("Selecionar");?> <?php $this->dict('entities: parent space') ?>"
                    ...
                    data-allow-clear="1"
                    title="<?php \MapasCulturais\i::_e("Selecionar");?> <?php $this->dict('entities: parent space') ?>"
                    data-value="<?php if($entity->parent) echo $entity->parent->id; ?>"
                    data-value-name="<?php if($entity->parent) echo $entity->parent->name; ?>"
              ><?php if($entity->parent) echo $entity->parent->name; ?></span>
        <?php endif; ?>
    ...
    <span class="warning pending js-pending-parent hltip" data-hltip-classes="hltip-warning" hltitle="<?php \MapasCulturais\i::_e("Aguardando confirmação");?>" <?php if(!$child_entity_request) echo 'style="display:none"'?> ></span>
