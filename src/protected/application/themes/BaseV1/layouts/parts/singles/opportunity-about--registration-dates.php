...
<?php if($editable || $entity->registrationFrom): ?>
    <div class="registration-dates clear">
        ...
        <?php if($entity->canUser('@control')):?>
            <?php \MapasCulturais\i::_e("Data para publicação automática dos resultados");?>
            <strong <?php if($editable): ?> class="js-editable" <?php endif; ?> data-type="date" data-yearrange="2000:+25" data-viewformat="dd/mm/yyyy" data-edit="publishTimestamp" <?php echo $entity->publishTimestamp ? "data-value='".$entity->publishTimestamp->format('Y-m-d') . "'" : ''?> data-timepicker="#publishTimestamp_time" data-showbuttons="false" data-emptytext="<?php \MapasCulturais\i::esc_attr_e("DD\MM\YYYY");?>"><?php echo $entity->publishTimestamp ? $entity->publishTimestamp->format('d/m/Y') : \MapasCulturais\i::__("DD\MM\YYYY"); ?></strong>
            <?php \MapasCulturais\i::_e("às");?>
            <strong <?php if($editable): ?> class="js-editable" id="publishTimestamp_time" <?php endif; ?> data-datetime-value="<?php echo $entity->publishTimestamp ? $entity->publishTimestamp->format('Y-m-d H:i') : ''; ?>" data-placeholder="<?php \MapasCulturais\i::esc_attr_e("23:59");?>" data-emptytext="<?php \MapasCulturais\i::esc_attr_e("Horário");?>"><?php echo $entity->publishTimestamp ? $entity->publishTimestamp->format('H:i') : \MapasCulturais\i::__("Horário"); ?></strong>
        <?php endif?>
        </div>
<?php endif; ?>
